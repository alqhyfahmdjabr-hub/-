...
type Tab = 'generator' | 'preview' | 'settings';
...
export default function App() {
  ...
  // Store Info State (Persisted)
  const [storeInfo, setStoreInfo] = useState<StoreInfo>(() => {
    const saved = localStorage.getItem('babel_store_info');
    return saved ? JSON.parse(saved) : { name: 'مجوهرات بابل', contacts: '', branches: '' };
  });
  ...
  // --- Effects ---
  useEffect(() => {
    localStorage.setItem('babel_store_info', JSON.stringify(storeInfo));
  }, [storeInfo]);
